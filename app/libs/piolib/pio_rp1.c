...
static pio_sm_config rp1_pio_get_default_sm_config(PIO unused)
{
    pio_sm_config c = { { 0 } };
...
static uint rp1_pio_encode_delay(PIO unused, uint cycles)
{
    return _pio_encode_delay(cycles);
}

static uint rp1_pio_encode_sideset(PIO unused, uint sideset_bit_count, uint value)
{
    return _pio_encode_sideset(sideset_bit_count, value);
}

static uint rp1_pio_encode_sideset_opt(PIO unused, uint sideset_bit_count, uint value)
{
    return _pio_encode_sideset_opt(sideset_bit_count, value);
}

static uint rp1_pio_encode_jmp(PIO unused, uint addr)
{
    return _pio_encode_jmp(addr);
}

static uint rp1_pio_encode_jmp_not_x(PIO unused, uint addr)
{
    return _pio_encode_jmp_not_x(addr);
}

static uint rp1_pio_encode_jmp_x_dec(PIO unused, uint addr)
{
    return _pio_encode_jmp_x_dec(addr);
}

static uint rp1_pio_encode_jmp_not_y(PIO unused, uint addr)
{
    return _pio_encode_jmp_not_y(addr);
}

static uint rp1_pio_encode_jmp_y_dec(PIO unused, uint addr)
{
    return _pio_encode_jmp_y_dec(addr);
}

static uint rp1_pio_encode_jmp_x_ne_y(PIO unused, uint addr)
{
    return _pio_encode_jmp_x_ne_y(addr);
}

static uint rp1_pio_encode_jmp_pin(PIO unused, uint addr)
{
    return _pio_encode_jmp_pin(addr);
}

static uint rp1_pio_encode_jmp_not_osre(PIO unused, uint addr)
{
    return _pio_encode_jmp_not_osre(addr);
}

static uint rp1_pio_encode_wait_gpio(PIO unused, bool polarity, uint gpio)
{
    return _pio_encode_wait_gpio(polarity, gpio);
}

static uint rp1_pio_encode_wait_pin(PIO unused, bool polarity, uint pin)
{
    return _pio_encode_wait_pin(polarity, pin);
}

static uint rp1_pio_encode_wait_irq(PIO unused, bool polarity, bool relative, uint irq)
{
    return _pio_encode_wait_irq(polarity, relative, irq);
}

static uint rp1_pio_encode_in(PIO unused, enum pio_src_dest src, uint count)
{
    return _pio_encode_in(src, count);
}

static uint rp1_pio_encode_out(PIO unused, enum pio_src_dest dest, uint count)
{
    return _pio_encode_out(dest, count);
}

static uint rp1_pio_encode_push(PIO unused, bool if_full, bool block)
{
    return _pio_encode_push(if_full, block);
}

static uint rp1_pio_encode_pull(PIO unused, bool if_empty, bool block)
{
    return _pio_encode_pull(if_empty, block);
}

static uint rp1_pio_encode_mov(PIO unused, enum pio_src_dest dest, enum pio_src_dest src)
{
    return _pio_encode_mov(dest, src);
}

static uint rp1_pio_encode_mov_not(PIO unused, enum pio_src_dest dest, enum pio_src_dest src)
{
    return _pio_encode_mov_not(dest, src);
}

static uint rp1_pio_encode_mov_reverse(PIO unused, enum pio_src_dest dest, enum pio_src_dest src)
{
    return _pio_encode_mov_reverse(dest, src);
}

static uint rp1_pio_encode_irq_set(PIO unused, bool relative, uint irq)
{
    return _pio_encode_irq_set(relative, irq);
}

static uint rp1_pio_encode_irq_wait(PIO unused, bool relative, uint irq)
{
    return _pio_encode_irq_wait(relative, irq);
}

static uint rp1_pio_encode_irq_clear(PIO unused, bool relative, uint irq)
{
    return _pio_encode_irq_clear(relative, irq);
}

static uint rp1_pio_encode_set(PIO unused, enum pio_src_dest dest, uint value)
{
    return _pio_encode_set(dest, value);
}

static uint rp1_pio_encode_nop(PIO unused)
{
    return _pio_encode_nop();
}

static int rp1_ioctl(PIO unused, int request, void *args)
{
    RP1_PIO rp = (RP1_PIO)unused;
...
static int rp1_pio_sm_config_xfer(PIO unused, uint sm, uint dir, uint buf_size, uint buf_count)
{
    ...
    if (buf_size > 0xffff || buf_count > 0xffff)
        err = rp1_ioctl(unused, PIO_IOC_SM_CONFIG_XFER32, &args32);
    else
        err = rp1_ioctl(unused, PIO_IOC_SM_CONFIG_XFER, &args);
    return err;
}

static int rp1_pio_sm_xfer_data(PIO unused, uint sm, uint dir, uint data_bytes, void *data)
{
    ...
    if (data_bytes > 0xffff)
        err = rp1_ioctl(unused, PIO_IOC_SM_XFER_DATA32, &args32);
    else
        err = rp1_ioctl(unused, PIO_IOC_SM_XFER_DATA, &args);
    return err;
}

static bool rp1_pio_can_add_program_at_offset(PIO unused, const pio_program_t *program, uint offset)
{
    ...
    err = rp1_ioctl(unused, PIO_IOC_CAN_ADD_PROGRAM, &args);
    return (err > 0);
}

static uint rp1_pio_add_program_at_offset(PIO unused, const pio_program_t *program, uint offset)
{
    struct rp1_pio_add_program_args args = { .num_instrs = program->length, .origin = program->origin };
    valid_params_if(PIO, offset < RP1_PIO_INSTRUCTION_COUNT || offset == PIO_ORIGIN_ANY);
    valid_params_if(PIO, program->length <= RP1_PIO_INSTRUCTION_COUNT);
    ...
    if (offset != PIO_ORIGIN_ANY)
        args.origin = offset;
    memcpy(args.instrs, program->instructions, program->length * sizeof(uint16_t));
    return rp1_ioctl(unused, PIO_IOC_ADD_PROGRAM, &args);
}

static bool rp1_pio_remove_program(PIO unused, const pio_program_t *program, uint offset)
{
    ...
    return !rp1_ioctl(unused, PIO_IOC_REMOVE_PROGRAM, &args);
}
...
static bool rp1_pio_sm_claim(PIO unused, uint sm)
{
    struct rp1_pio_sm_claim_args args = { .mask = (1 << sm) };
    check_sm_param(sm);
    return (rp1_ioctl(unused, PIO_IOC_SM_CLAIM, &args) >= 0);
}

static bool rp1_pio_sm_claim_mask(PIO unused, uint mask)
{
...
static bool rp1_pio_sm_unclaim(PIO unused, uint sm)
{
    struct rp1_pio_sm_claim_args args = { .mask = (1 << sm) };
    check_sm_param(sm);
    return !rp1_ioctl(unused, PIO_IOC_SM_UNCLAIM, &args);
}

static int rp1_pio_sm_claim_unused(PIO unused, bool required)
{
    struct rp1_pio_sm_claim_args args = { .mask = 0 };
    int sm = rp1_ioctl(unused, PIO_IOC_SM_CLAIM, &args);
    if (sm < 0 && required)
        pio_panic("No PIO state machines are available");
    return sm;
}

static bool rp1_pio_sm_is_claimed(PIO unused, uint sm)
{
    struct rp1_pio_sm_claim_args args = { .mask = (1 << sm) };
    check_sm_param(sm);
    int err = rp1_ioctl(unused, PIO_IOC_SM_IS_CLAIMED, &args);
    return (err > 0);
}

static void rp1_pio_sm_init(PIO unused, uint sm, uint initial_pc, const pio_sm_config *config)
{
    smc_to_rp1(config, c);
    ...
    (void)rp1_ioctl(unused, PIO_IOC_SM_INIT, &args);
}

static void rp1_pio_sm_set_config(PIO unused, uint sm, const pio_sm_config *config)
{
    smc_to_rp1(config, c);
    struct rp1_pio_sm_init_args args = { .sm = sm, .config = *c };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_CONFIG, &args);
}

static void rp1_pio_sm_exec(PIO unused, uint sm, uint instr, bool blocking)
{
    struct rp1_pio_sm_exec_args args = { .sm = sm, .instr = instr, .blocking = blocking };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_EXEC, &args);
}

static void rp1_pio_sm_clear_fifos(PIO unused, uint sm)
{
    struct rp1_pio_sm_clear_fifos_args args = { .sm = sm };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_CLEAR_FIFOS, &args);
}
...
static void rp1_pio_sm_set_clkdiv_int_frac(PIO unused, uint sm, uint16_t div_int, uint8_t div_frac)
{
    struct rp1_pio_sm_set_clkdiv_args args = { .sm = sm, .div_int = div_int, .div_frac = div_frac };

    check_sm_param(sm);
    invalid_params_if(PIO, div_int == 0 && div_frac != 0);
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_CLKDIV, &args);
}

static void rp1_pio_sm_set_clkdiv(PIO unused, uint sm, float div)
{
    uint16_t div_int;
    uint8_t div_frac;
    ...
    rp1_pio_sm_set_clkdiv_int_frac(unused, sm, div_int, div_frac);
}

static void rp1_pio_sm_set_pins(PIO unused, uint sm, uint32_t pin_values)
{
    struct rp1_pio_sm_set_pins_args args = { .sm = sm, .values = pin_values, .mask = GPIOS_MASK };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_PINS, &args);
}

static void rp1_pio_sm_set_pins_with_mask(PIO unused, uint sm, uint32_t pin_values, uint32_t pin_mask)
{
...
static void rp1_pio_sm_set_pindirs_with_mask(PIO unused, uint sm, uint32_t pin_dirs, uint32_t pin_mask)
{
    ...
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_PINDIRS, &args);
}

static void rp1_pio_sm_set_consecutive_pindirs(PIO unused, uint sm, uint pin_base, uint pin_count, bool is_out)
{
...
static void rp1_pio_sm_set_enabled(PIO unused, uint sm, bool enabled)
{
    struct rp1_pio_sm_set_enabled_args args = { .mask = (1 << sm), .enable = enabled };
    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_ENABLED, &args);
}

static void rp1_pio_sm_set_enabled_mask(PIO unused, uint32_t mask, bool enabled)
{
...
static void rp1_pio_sm_restart(PIO unused, uint sm)
{
    struct rp1_pio_sm_restart_args args = { .mask = (1 << sm) };
    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_RESTART, &args);
}

static void rp1_pio_sm_restart_mask(PIO unused, uint32_t mask)
{
...
static void rp1_pio_sm_clkdiv_restart_mask(PIO unused, uint32_t mask)
{
    struct rp1_pio_sm_restart_args args = { .mask = (uint16_t)mask };

    check_sm_mask(mask);
    (void)rp1_ioctl(unused, PIO_IOC_SM_CLKDIV_RESTART, &args);
}

static void rp1_pio_sm_enable_sync(PIO unused, uint32_t mask)
{
    struct rp1_pio_sm_enable_sync_args args = { .mask = (uint16_t)mask };

    check_sm_mask(mask);
    (void)rp1_ioctl(unused, PIO_IOC_SM_ENABLE_SYNC, &args);
}

static void rp1_pio_sm_put(PIO unused, uint sm, uint32_t data, bool blocking)
{
    struct rp1_pio_sm_put_args args = { .sm = (uint16_t)sm, .blocking = blocking, .data = data };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_PUT, &args);
}

static uint32_t rp1_pio_sm_get(PIO unused, uint sm, bool blocking)
{
    struct rp1_pio_sm_get_args args = { .sm = (uint16_t)sm, .blocking = blocking };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_GET, &args);
    return args.data;
}

static void rp1_pio_sm_set_dmactrl(PIO unused, uint sm, bool is_tx, uint32_t ctrl)
{
    struct rp1_pio_sm_set_dmactrl_args args = { .sm = sm, .is_tx = is_tx, .ctrl = ctrl };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_SET_DMACTRL, &args);
}

static bool rp1_pio_sm_is_rx_fifo_empty(PIO unused, uint sm)
{
    struct rp1_pio_sm_fifo_state_args args = { .sm = sm, .tx = false };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_FIFO_STATE, &args);
    return args.empty;
}

static bool rp1_pio_sm_is_rx_fifo_full(PIO unused, uint sm)
{
    struct rp1_pio_sm_fifo_state_args args = { .sm = sm, .tx = false };
...
static uint rp1_pio_sm_get_rx_fifo_level(PIO unused, uint sm)
{
    struct rp1_pio_sm_fifo_state_args args = { .sm = sm, .tx = false };
...
static bool rp1_pio_sm_is_tx_fifo_empty(PIO unused, uint sm)
{
...
static bool rp1_pio_sm_is_tx_fifo_full(PIO unused, uint sm)
{
    struct rp1_pio_sm_fifo_state_args args = { .sm = sm, .tx = true };
...
static uint rp1_pio_sm_get_tx_fifo_level(PIO unused, uint sm)
{
    struct rp1_pio_sm_fifo_state_args args = { .sm = sm, .tx = true };
...
static void rp1_pio_sm_drain_tx_fifo(PIO unused, uint sm)
{
    struct rp1_pio_sm_clear_fifos_args args = { .sm = sm };

    check_sm_param(sm);
    (void)rp1_ioctl(unused, PIO_IOC_SM_DRAIN_TX, &args);
}

static void rp1_smc_set_out_pins(PIO unused, pio_sm_config *config, uint out_base, uint out_count)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_set_pins(PIO unused, pio_sm_config *config, uint set_base, uint set_count)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_in_pins(PIO unused, pio_sm_config *config, uint in_base)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_sideset_pins(PIO unused, pio_sm_config *config, uint sideset_base)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_sideset(PIO unused, pio_sm_config *config, uint bit_count, bool optional, bool pindirs)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_clkdiv_int_frac(PIO unused, pio_sm_config *config, uint16_t div_int, uint8_t div_frac)
{
    smc_to_rp1(config, c);
    invalid_params_if(PIO, div_int == 0 && div_frac != 0);
    c->clkdiv =
...
static void rp1_smc_set_clkdiv(PIO unused, pio_sm_config *config, float div)
{
    uint16_t div_int;
    uint8_t div_frac;
    rp1_pio_calculate_clkdiv_from_float(div, &div_int, &div_frac);
...
static void rp1_smc_set_wrap(PIO unused, pio_sm_config *config, uint wrap_target, uint wrap)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_jmp_pin(PIO unused, pio_sm_config *config, uint pin)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_in_shift(PIO unused, pio_sm_config *config, bool shift_right, bool autopush, uint push_threshold)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_out_shift(PIO unused, pio_sm_config *config, bool shift_right, bool autopull, uint pull_threshold)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_fifo_join(PIO unused, pio_sm_config *config, enum pio_fifo_join join)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_out_special(PIO unused, pio_sm_config *config, bool sticky, bool has_enable_pin, uint enable_pin_index)
{
    smc_to_rp1(config, c);
...
static void rp1_smc_set_mov_status(PIO unused, pio_sm_config *config, enum pio_mov_status_type status_sel, uint status_n)
{
    smc_to_rp1(config, c);
...
static uint32_t rp1_clock_get_hz(PIO __unused__, enum clock_index clk_index)
{
...
static void rp1_gpio_init(PIO unused, uint gpio)
{
    struct rp1_gpio_init_args args = { .gpio = gpio };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_INIT, &args);
}

static void rp1_gpio_set_function(PIO unused, uint gpio, enum gpio_function fn)
{
    struct rp1_gpio_set_function_args args = { .gpio = gpio, .fn = fn };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_FUNCTION, &args);
}

static void rp1_gpio_set_pulls(PIO unused, uint gpio, bool up, bool down)
{
    struct rp1_gpio_set_pulls_args args = { .gpio = gpio, .up = up, .down = down };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_PULLS, &args);
}

static void rp1_gpio_set_outover(PIO unused, uint gpio, uint value)
{
    struct rp1_gpio_set_args args = { .gpio = gpio, .value = value };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_OUTOVER, &args);
}

static void rp1_gpio_set_inover(PIO unused, uint gpio, uint value)
{
    struct rp1_gpio_set_args args = { .gpio = gpio, .value = value };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_INOVER, &args);
}

static void rp1_gpio_set_oeover(PIO unused, uint gpio, uint value)
{
    struct rp1_gpio_set_args args = { .gpio = gpio, .value = value };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_OEOVER, &args);
}

static void rp1_gpio_set_input_enabled(PIO unused, uint gpio, bool enabled)
{
    struct rp1_gpio_set_args args = { .gpio = gpio, .value = enabled };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_INPUT_ENABLED, &args);
}

static void rp1_gpio_set_drive_strength(PIO unused, uint gpio, enum gpio_drive_strength drive)
{
    struct rp1_gpio_set_args args = { .gpio = gpio, .value = drive };

    valid_params_if(PIO, gpio < RP1_PIO_GPIO_COUNT);
    (void)rp1_ioctl(unused, PIO_IOC_GPIO_SET_DRIVE_STRENGTH, &args);
}

static void rp1_pio_gpio_init(PIO unused, uint pin)
{
    valid_params_if(PIO, pin < RP1_PIO_GPIO_COUNT);
    rp1_gpio_set_function(unused, pin, RP1_GPIO_FUNC_PIO);
}
